...
pub fn dfs_cycle(graph: &Graph, start : usize) -> Vec<usize> {
    let mut cycle = vec![];
...
pub fn measure_perm(points: &Points, perm: &[usize]) -> u32 {
    assert!(perm.len() >= 2);
    ...
    sum + points.list[*perm.last().unwrap()].dist(&points.list[*perm.first().unwrap()])
}

pub fn random_perm(n: usize) -> Vec<usize> {
    use::rand::prelude::*;
...
mod tests {
    ...
    fn xqf131_test() {
        ...
        let cycle = dfs_cycle(&mst, 0);
        let length = measure_perm(&points, &cycle);
